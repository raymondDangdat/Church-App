    ...
    private void declineApplication(){
        ...
        members.child(applicant_id).child("status").setValue("declined").addOnCompleteListener(
                new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()){
                            dialog.dismiss();
                            Toast.makeText(ApplicantProfile.this, "Application Declined!", Toast.LENGTH_SHORT).show();
                            startActivity(new Intent(ApplicantProfile.this, MembershipApplications.class));
                            finish();
                        }
                    }
                }
            ...
            public void onFailure(@NonNull Exception e) {
                dialog.dismiss();
                Toast.makeText(ApplicantProfile.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
        ...
        members.child(applicant_id).child("status").setValue("member").addOnCompleteListener(
                new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()){
                            dialog.dismiss();
                            Toast.makeText(ApplicantProfile.this, "Member Approved!", Toast.LENGTH_SHORT).show();
                            startActivity(new Intent(ApplicantProfile.this, MembershipApplications.class));
                            finish();
                        }
                    }
